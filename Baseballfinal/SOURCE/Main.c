...
int __TS1(void);
int __TouchIn(void);
void baseset(int *base);
void inout(int *input);
int strike(int *input,int *base);
int ball(int *input,int *base);
void baseloop(void);
int num=4;
...
void baseset(int *base) //πËø≠¿ª ∆˜¿Œ≈Õ∑Œ πﬁæ∆ ∞™ ≥◊∞≥∏¶ π›»Ø base[0]¿∫ 1π¯¬∞¿⁄∏Æ ~~~¿Ã∑±Ωƒ 
{
	base[0]=rand()%9+1;
	while(1)
	{
	...
	{
		base[2]=rand()%9+1;
		if(base[0]!=base[2])	{if(base[1]!=base[2])	break;	}	
	}
	while(1)
	{
		base[3]=rand()%9+1;
		if(base[0]!=base[3])	{if(base[1]!=base[3])	{if(base[2]!=base[3])	break;	}}	
	}
	Lcd_Printf(260,0,WHITE,BLACK,1,1," BALL SETTING COMPLITE ");
}	
//∞™ πﬁæ∆º≠ char¿ª intπËø≠∑Œ ∫Ø»Ø
void inout(int *input)
{
	char a[4]={0};
	int i;
	int xi=4;
	/*printf("%d¿⁄∏Æ º˝¿⁄∏¶ ¿‘∑¬«œººø‰ : ",num);
	scanf("%s",a);
	*/
	...
	while(1){ ; if(Touch_Pressed != 1) { break;}}
	}
	
	
	//Lcd_Printf(200,0+16*i,WHITE,BLACK,2,2,input[i]);	}
...
void baseloop(void)
{
	...
	// ∏ﬁ¿Œ ∫£¿ÃΩ∫∫º »≠∏È √‚∑¬ «‘ºˆ
	// ≈Õƒ°«‘ºˆ 
	//printf("3¿⁄∏Æ ∫£¿ÃΩ∫∫º¿ª «œΩ√∑¡∏È 1, 4¿⁄∏Æ ∫£¿ÃΩ∫∫º¿ª «œΩ√∑¡∏È 2∏¶ ¿‘∑¬«œΩ Ω√ø¿\n");
	//scanf("%d",&num);
	//num+=2;
	for(j=0; j<272; j++)
	{
	...
	baseset(base);
	k=0;
	//printf("set : %d %d %d %d\n",base[0],base[1],base[2],base[3]);   // »Æ¿ŒøÎ √‚∑¬ 
	while(1)
	{	
		count++;
		inout(input);
Lcd_Printf(250+0,25+200,WHITE,BLACK,1,1,"%d",base[0]);
Lcd_Printf(250+9,25+200,WHITE,BLACK,1,1,"%d",base[1]);
Lcd_Printf(250+18,25+200,WHITE,BLACK,1,1,"%d",base[2]);
Lcd_Printf(250+27,25+200,WHITE,BLACK,1,1,"%d",base[3]);

		
		
		
Lcd_Printf(250+0,25+30*k,WHITE,BLACK,1,1,"%d",input[0]);
Lcd_Printf(250+9,25+30*k,WHITE,BLACK,1,1,"%d",input[1]);
Lcd_Printf(250+18,25+30*k,WHITE,BLACK,1,1,"%d",input[2]);
Lcd_Printf(250+27,25+30*k,WHITE,BLACK,1,1,"%d",input[3]);
		...
		Lcd_Printf(250+100,25+30*k,BLACK,WHITE,1,1,"%d",s);
		Lcd_Printf(270+100,25+30*k,BLACK,WHITE,1,1,"s");
		Lcd_Printf(300+100,25+30*k,BLACK,WHITE,1,1,"%d",b);
		Lcd_Printf(320+100,25+30*k,BLACK,WHITE,1,1,"b");
		//printf("%d s  %d b ¿‘¥œ¥Ÿ\n\n",s,b);
		k++;
		if(s==num) {/*printf("√‡«œ«’¥œ¥Ÿ!! %dπ¯ ∏∏ø° ∏¬√ﬂºÃΩ¿¥œ¥Ÿ",count);*/
		Lcd_Printf(270,35+30*k,WHITE,RED,2,1," **CLEAR** ");
		 break;} 
